...
| G2 speed vs upstream | Beat upstream on **every** cell of S1, S2, S4, S5 (win-rate ≥ 19/20, z > 2, above the null-arm floor); headline cells S1 DOM parse ≥ **1.8×**, S1 struct parse ≥ **1.4×**, S1 struct stringify ≥ **1.3×**, S4 struct parse ≥ **1.4×** with allocs per parse ≤ upstream's; S3 parse ≥ 1.0× (no regression; no win claimed); no cell anywhere below 0.97× | **measurable at last** -- S4 struct parse was unmeasurable until the fixtures landed at M1-D, so that bar could not even be read. Progress on the others: S1 struct stringify **1.208x** (B3), S1 DOM parse still the long pole and allocation-bound (B10) |
...
| M3 | ✅ **LANDED 2026-09-10.** The `-accel` island: `rusty_json_turbo-accel`, the one crate where `unsafe` is allowed, carrying SSE2 and AVX2 twins of the whitespace and escape scanners with the scalar oracle written FIRST, kept permanently, and reachable in production via `RJT_ISA=scalar`. No deps, no build script. Dispatch at the surface, `RJT_ISA` override, `isa=` on every method line beside the machine's ceiling | **SSE2 SHIPS, AVX2 DOES NOT -- measured, not cautious.** AVX2 won on citm and LOST on twitter (0.958x best-of-N) and canada; SSE2 won on EVERY cell. The longest whitespace run in the corpus is **29 bytes**, so a 32-byte step is never fully used while its costs are paid every run. AVX2 stays reachable for re-measurement elsewhere. | Twins vs oracle over all 256 byte values at every offset across the 8/16/32-byte boundaries (>500k assertions per scanner), the `0x0B`/`0x0C` trap, the signed-compare trap, a poison test proving the gate discriminates, and oracle+soak+S4 byte-identical at ALL FOUR rungs; compiles for wasm32/aarch64-musl/x86_64-gnu with `--no-default-features` | **citm scan 1.105x (61/61), s4-media-probe scan 1.094x, citm struct-parse 1.075x; twitter struct-stringify 1.062x, dom-stringify 1.060x, citm struct-stringify 1.053x** -- controls flat, nothing below 0.987x. Two things built and thrown away: an escalation gate (cost citm half its win, did not fix twitter) and, kept, a length guard that turned citm struct-stringify from 0.970x into 1.053x |

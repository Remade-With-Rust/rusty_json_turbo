...
| B5 | **PROMOTED 2026-09-09, against this plan's own gate.** The gate said count the calls first and do not build if already near one per token. Measured: **2.6 bytes per sink call** on citm (189,201 calls for 25,869 keys, ~7.3 per key), 5.0 on twitter, 6.2 on canada -- the serializer hands the writer a quote, a colon and a comma as separate calls. Its claim is now narrow and clear: not allocations (M1-A: stringify allocates zero times) and not copies (the asm census: `ser.rs` owns **zero** `mem*` calls), but **call count alone** | `ser.rs`, all `write_all` sites | codec-memory-copies | output bytes identical | priced by `CountingWriter` in the harness, no library change |

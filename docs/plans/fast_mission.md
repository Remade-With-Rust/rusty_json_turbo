...
| G5 portability | 8-target check matrix green; `--no-default-features --features alloc` tested; wasm32 test under wasmtime; NEON + simd128 kernels present with per-arch census, or a written note why not | ✅ **MET 2026-09-10.** Matrix green on nine targets × four configurations (library default / alloc / alloc+every-feature, plus the island), 36 checks, zero errors. `--no-default-features --features alloc` is now **RUN**, not only checked, in four feature configurations -- it had been checked on eight targets and executed on none, so its `#[cfg(not(feature = "std"))]` branches had never run. **wasm32-wasip1 under wasmtime 48 executes the real suite**: 249 library tests, the differential oracle over the whole corpus, the soak, S6 cold paths, B13, B14, struct-stringify and the no_std probe -- all green. NEON and simd128: a **written note**, argued from the corpus (mean whitespace run 7.2 bytes, longest 29; 40% of escape scan steps are already tail bytes) and from the fact that we have no aarch64 hardware to measure on -- LEDGER 2026-09-10 M6. Per-arch census asserted on x86_64-linux, x86_64-windows, aarch64-macos and wasm32, and **every counter reading on wasm32 is byte-for-byte identical to x86_64** |
| G6 reachability | Census = 100% of scanned/written bytes through the intended kernel on every arch shipped, on S1, S2, S4, S5 | 🟡 **PARTIALLY MET 2026-09-10.** `tests/m6_arch_census.rs` asserts reachability per architecture and does it EXACTLY rather than by ratio: it predicts the escape scanner's step count from the document's own string lengths (`L / 8` wide steps plus `L % 8` tail steps, because `scan_to_escape` has no peel) and compares. It predicts 100,650 on twitter and the counter reads 101,382 -- a 0.7% excess exactly accounted for by 1,228 escape restarts, against 367,917 for the per-byte fallback. Also asserts the whitespace scanner ran (169,287 runs, 1,226,905 bytes on citm) and the eight-digit fold HIT (108,878/331,084 on canada). **The census's first version was wrong** -- it assumed 8 bytes per step and failed at 3.63, because twitter's mean string is 20 bytes; the counter caught the instrument, not the code. Still to do: extend to S4/S5 columns and state the 100% claim as a byte-accounting identity rather than a per-counter assertion |
...
| B11 (v1.x) | Arena `Value` as an additional type | new module | footprint-decomposition + codec-cache-tiles (gated by S8) | `to_string` identical to `serde_json::Value`'s | 🟡 **CEILING MEASURED AND LEVERS PRICED 2026-09-10; not built.** The `scan` column builds NOTHING, so `dom-parse` above it is the price of building a `Value`: **70%-85% of a DOM parse** -- a 3.29x ceiling on citm, 3.37x canada, 3.78x twitter, **6.89x on s4-frame-telemetry**. By contrast whitespace, after B1+B1s+the island, has 1.02x-1.19x left (and 0.95x on the minified fixture). **The largest remaining target in the project is not the parser.** `tests/b11_price.rs` prices three levers with one generic visitor and four storage models, so only the storage differs and the probe's own overhead cancels: interned keys, a sorted-`Vec` map, inline short strings. All three together measure a **median ~1.45x against the shipped `Value`** across NINE runs -- but the spread is **0.97x-1.74x** and two readings fall below 1.00, because this probe's arms run block-wise rather than paired, so interference lands on whichever arm is running. **Indicative, not quotable**: enough to justify building, not enough to publish, and it was briefly written into the README as though it were before being corrected. `canada` is a natural control (8 key occurrences, 1.3x reuse -> 0.95x-1.16x, straddling 1.00). Interning pays (1.19x-1.34x on the 1,418.7x-reuse fixture) **despite** `preserve_order`'s IndexMap losing 1.16x-1.45x -- same hash per key, but an interner allocates 33 times instead of 46,816. **The law: a shared cost is only a verdict when the thing it buys is the same.** Split: the MAP is shippable and non-breaking (a sorted Vec iterates in BTreeMap order), interning and inline strings change the public API |
...
| M6 | Portability: NEON + simd128 twins with per-arch census (qemu-aarch64 + wasmtime in CI), `--no-default-features` tests on every target, wasm32 test job | ✅ **G5 MET 2026-09-10**, and the twins are a written note rather than code -- see the G5 row. Delivered: a `wasm` CI job running the library suite, the oracle, the soak and every brick gate on `wasm32-wasip1`; the no_std probe turned from a three-line re-export into one that RUNS real assertions (including exact error text and line/column) in four feature configurations; the per-arch census on three OSes plus wasm. Three defects found by running what had only been checked: the harness could not be cross-compiled at all (`env!("CARGO_MANIFEST_DIR")` is an absolute HOST path WASI cannot open), one test traps on WASI and would have taken the whole binary with it (`TcpListener` + threads, now gated by name beside its existing `not(miri)`), and no_std had never executed. The **browser demo checksum is MET** as well: `crates/rusty_json_turbo-wasmdemo` embeds three corpus documents, parses and re-serializes them inside `wasm32-unknown-unknown` -- the target a page actually loads, with no filesystem, clock or WASI underneath -- and its FNV-1a of the output bytes equals native exactly on all three, including **2,063,469 bytes of re-serialized `canada`**, the float family where a difference would be a silent wrong answer. The parser is linked alloc-only there, and the module is asserted to need **no host imports at all**. A **qemu-user job for `aarch64-unknown-linux-gnu`** closes the last gap on the platform list -- library suite, oracle, census and `no_std` probe, correctness only (a qemu timing is meaningless and none is taken). **Honest caveat: that job is the one item here not executed**, because there is no aarch64 hardware or qemu on the dev box; what is verified locally is that the bench crate and the census compile clean for `aarch64-unknown-linux-gnu` and `-musl`, leaving only the standard linker-and-qemu setup for the first CI run to prove. Written as a real gate, not `continue-on-error` |
| M7 | Consumer swap | 🟡 **IN PROGRESS 2026-09-10, and the mechanism is settled.** The plan said `via package =` for three repos and `[patch.crates-io]` only for mata-master. **That is backwards, and the reverse-dependency trees say why:** of the 22 repositories that depend on `serde_json`, most reach it through a THIRD-PARTY crate as well as directly (axum, cozo, dioxus-desktop, candle-transformers, lambda_http, spacedb-crdt). A rename converts only the JSON a repo writes itself, leaves every third-party copy on upstream, and makes the two **different types** -- so a `Value` crossing that boundary would not compile, and the fork never reaches the serialization a web service actually spends its time on. `[patch.crates-io]` is the right tool for ALL of them: one table, whole graph, **zero per-member edits** (mata-master has 107 declarations). It needs a package named `serde_json`, because `[patch]` matches on PACKAGE name and ours is `rusty_json_turbo` -- hence `crates/serde_json-shim`, a three-line re-export (not a second compilation, which would produce types that do not unify) gated by code that only COMPILES if the types are identical | the shim, plus `[patch.crates-io]` in each consumer's workspace roots | codec-measurement | **the shim exposed a hole open since M0**: the harness declares its oracle as `{ package = "serde_json", version = "=1.0.151" }`, and once a package of that name exists here the wrong answer would silently make every differential result a comparison of us against ourselves. Nothing checked it -- `oracle_reports_a_difference` tests the comparison FUNCTION. `tests/m7_oracle_identity.rs` now proves it with a fact no naming trick can fake: our counters are statics inside our crate, so the oracle arm must leave every one at zero (measured: it does, while our arm reports 169,287 whitespace runs on the same document), and the oracle must be the pinned registry version, not a local path | **rusty_time: 217 tests green, 124 fixtures / 6,465 number tokens, 0 mismatches. mid: 184 green. terra: 81 green. mata-maestro: 49. thesoilapp: 13. rusty_dds: 89** (its 6 encode-hash failures proved PRE-EXISTING by a control run with the patch removed -- identical output both ways). Patched and resolution-verified but not yet suite-validated here: FFai, mata-master, remade_ffmpeg_rs, and janus's espino / rusty_esp_iroh / rusty_esp_mid. **8 repos blocked by uncommitted work in their trees** and deliberately untouched |

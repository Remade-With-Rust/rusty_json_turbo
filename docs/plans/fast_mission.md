...
| B5 | `to_vec` sink specialisation: write into `Vec` spare capacity with one reserve per token; `itoa`/`zmij` buffers written in place; size memo for the pre-size | `ser.rs:2213`, all `write_all` sites | codec-memory-copies | output bytes identical | **repriced at M1-A**: the census says a stringify op into a pre-sized buffer allocates **zero** times, so there are no allocations for this brick to remove and no allocator effect to ride. It must win on `write_all` COUNT alone. Count the calls per S1 stringify first; if the count is already near one per token, do not build |
...
| B10 | `Value` DOM: build `Map` from a sorted `Vec<(String, Value)>` (`BTreeMap::from_iter` on sorted input is O(n)) instead of per-entry `insert`; reserve `Vec` in `visit_seq` by a small probe | `value/de.rs:137-141` | codec-memory-copies | `Value` equality and identical sorted serialisation | **promoted at M1-A**: DOM parse is allocation-dominated on measured evidence — 20,834 allocations for a 632 KB document, one per ~31 input bytes, and swapping only the allocator moved the cell 1.53x. Two independent instruments now agree (this census; sonic-rs's arena-`Value` lead at M0). Probe: S1/S2 DOM parse with value construction discarded |

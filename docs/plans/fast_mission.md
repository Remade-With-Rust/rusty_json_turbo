...
| G2 speed vs upstream | Beat upstream on **every** cell of S1, S2, S4, S5 (win-rate ≥ 19/20, z > 2, above the null-arm floor); headline cells S1 DOM parse ≥ **1.8×**, S1 struct parse ≥ **1.4×**, S1 struct stringify ≥ **1.3×**, S4 struct parse ≥ **1.4×** with allocs per parse ≤ upstream's; S3 parse ≥ 1.0× (no regression; no win claimed); no cell anywhere below 0.97× | **MEASURED 2026-09-10 on the shipping default (island OFF): 12 of 15 cells beat upstream, ten at 20-21/21.** MET: S1 struct stringify **1.53x** (bar 1.3x); S3 no-regression (1.04x struct, 1.01x DOM); no S1-S3 json-benchmark cell below 0.97x (worst twitter struct-parse 1.009x, inside the null floor). **NOT MET: S1 DOM parse 1.04x (bar 1.8x) and S1 struct parse 0.99x (bar 1.4x) -- both now written up as ARCHITECTURAL, not open**: DOM parse is ~1 alloc per 31 bytes and needs a different `Value` type (v1.x arena), and the B6 probe showed FREE key dispatch is worth 0.6%. S4 struct parse NOT MET, and s4-frame-telemetry struct-parse/stringify LOSE ~1.14x at 0/11 with **the cause unexplained** -- every brick toggled off in turn moves it under 2% |

...
| B1s | Wide whitespace scan (SWAR 8-byte, then SSE2/AVX2 twins with the scalar loop as oracle) | `read.rs` `SliceRead::skip_whitespace` | codec-vectorize-kernel | byte-identical; `*_matches_scalar` over all 256 byte values × positions | **priced by B1**: whitespace costs 565,500 ns of citm's 878,500 ns scan and B1 recovered ~126,000 ns, so ~440,000 ns (78% of the whitespace cost, 50% of the scan) is still on the table |
